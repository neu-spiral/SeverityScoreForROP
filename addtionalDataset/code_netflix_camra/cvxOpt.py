...
def Log_Log(absDataOrigin,absLabels,cmpDataOrigin,cmpLabels, absWeight, lamda, normalizeWeight=0):
# This function uses both absolute label data and comparison data to train the logistic regression model.
...
# const : the logistic regression global constant.
    ...
    if normalizeWeight == 0:
        cmpWeight = 1.0 - absWeight
    elif normalizeWeight == 1:
        cmpWeight = float(1-absWeight)/float(cmpN)
        absWeight = float(absWeight)/float(absN)
    else :
        assert('normalizeWeight should be a binary value 0 or 1')
    absData = np.concatenate((np.array(absDataOrigin),np.ones([absN,1])),axis = 1)
