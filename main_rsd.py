...
def TrainSinleExp(alpha, labelSin,labelAbs, labelCmp,labelTrainPartition, labelTestPartition, cmpTrainPartition, cmpTestPartition, loss,num_iters=10000):
        ...
        for countFold in range(K):
            ...
            for lamda in lamdaWeights:
                ...
                if loss=='LogLog':
                    ...
                    betaSin, constSin = Log_SVM(XtrainSinL, YtrainSinL, XtrainSinC, YtrainSinC, alpha, lamda)
                elif loss == 'SVMLog':
                    betaSin, constSin = SVM_Log(XtrainSinL, YtrainSinL, XtrainSinC, YtrainSinC, alpha, lamda)
                elif loss == 'SVMSVM':
                    betaSin, constSin = SVM_SVM(XtrainSinL, YtrainSinL, XtrainSinC, YtrainSinC, alpha, lamda)
                else:
...
alphaWeights = [args.alpha]
lamdaWeights = [args.lam]
loss = args.loss
dataType = 'CNN'
nameBase = '../result/rop/rsd/'+loss+'/MS_RSD_'+loss+'_L1_CV1' + '_' +dataType
